...
class LocalFilesystemAdapter(FilesystemPort):
    ...
    async def discover_files(self, root_path: Path, include_patterns: Optional[List[str]] = None, exclude_patterns: Optional[List[str]] = None) -> List[Path]:
        """Discover files matching patterns."""
        return await self._crawler_service.discover_files(root_path, include_patterns, exclude_patterns)
    ...
    async def read_document(self, path: Path) -> Document:
        ...
        return Document(
            ...
            metadata={"file_size": stat.st_size, "encoding": "utf-8", "mime_type": self._get_mime_type(path)},
        )
