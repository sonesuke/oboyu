...
class SystemHealth:
    ...
    def record_successful_operation(self, operation_id: str, operation_type: str, duration: float = 0.0) -> None:
        ...
        """
        record = OperationRecord(operation_id=operation_id, operation_type=operation_type, timestamp=datetime.now(), success=True, duration_seconds=duration)
    ...
    def record_failed_operation(self, operation_id: str, operation_type: str, error_type: str, duration: float = 0.0) -> None:
        ...
        record = OperationRecord(
            operation_id=operation_id, operation_type=operation_type, timestamp=datetime.now(), success=False, duration_seconds=duration, error_type=error_type
        )
    ...
    def set_corruption_detected(self, severity: str = "medium") -> None:
        ...
        self._corruption_severity_counts[severity] = self._corruption_severity_counts.get(severity, 0) + 1
...
class IndexHealthMonitor(EventHandler):
    ...
    def _update_health_from_event(self, event: IndexEvent) -> None:
        ...
        if event.event_type == "indexing_completed":
            ...
            self.health_status.record_successful_operation(operation_id=event.operation_id, operation_type=event.event_type, duration=duration)
            ...
            self.health_status.record_failed_operation(
                operation_id=event.operation_id, operation_type=event.event_type, error_type=error_type, duration=duration
            )

        elif event.event_type == "database_cleared":
            self.health_status.record_successful_operation(operation_id=event.operation_id, operation_type=event.event_type)

        elif event.event_type == "database_clear_failed":
            error_type = getattr(event, "error_type", "unknown")
            self.health_status.record_failed_operation(operation_id=event.operation_id, operation_type=event.event_type, error_type=error_type)
