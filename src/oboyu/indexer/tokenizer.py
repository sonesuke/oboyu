...
class JapaneseTokenizer:
    ...
    def __init__(
        ...
        if not HAS_JAPANESE_TOKENIZER:
            raise ImportError("Japanese tokenizer dependencies not installed. Install with: pip install fugashi unidic-lite jaconv")
...
def create_tokenizer(
    ...
    if language == "ja" and not use_fallback:
        if HAS_JAPANESE_TOKENIZER:
            ...
            logger.warning("Japanese tokenizer not available, using fallback. For better results, install: pip install fugashi unidic-lite jaconv")
