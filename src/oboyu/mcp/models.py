            ...
            "examples": [
                {
                    ...
                    "top_k": 10,
                    "language": "ja",
                    "snippet_config": {"length": 200, "highlight_matches": True},
                },
                {"query": "Python async programming", "mode": "vector", "top_k": 5, "language": "en"},
            ]
        }
...
class MCPIndexRequest(BaseModel):
    ...
    db_path: Optional[str] = Field(default=None, description="Database file path")
    include_patterns: List[str] = Field(default_factory=lambda: ["*.txt", "*.md", "*.py", "*.rst"], description="File patterns to include")
    exclude_patterns: List[str] = Field(default_factory=lambda: ["*/node_modules/*", "*/venv/*"], description="Patterns to exclude")
