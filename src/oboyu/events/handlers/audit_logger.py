...
class AuditLogger(EventHandler):
    ...
    def _log_event_details(self, event: IndexEvent) -> None:
        ...
        if event.event_type == "indexing_started":
            logger.info(
                f"Indexing started: {getattr(event, 'document_count', 0)} documents, {getattr(event, 'total_size_bytes', 0)} bytes",
            ...
            logger.error(f"Indexing failed: {getattr(event, 'error', 'Unknown error')}", extra={"operation_id": event.operation_id})

        elif event.event_type == "database_cleared":
            logger.warning(f"Database cleared: {getattr(event, 'records_deleted', 0)} records deleted", extra={"operation_id": event.operation_id})

        elif event.event_type == "database_clear_failed":
            logger.error(f"Database clear failed: {getattr(event, 'error', 'Unknown error')}", extra={"operation_id": event.operation_id})

        elif event.event_type == "index_corruption_detected":
            logger.critical(
                f"Index corruption detected: {getattr(event, 'corruption_type', 'Unknown')} (severity: {getattr(event, 'severity', 'unknown')})",
                extra={"operation_id": event.operation_id},
            )
            ...
            if issues:
                logger.warning(f"Health check completed: {health_status}, issues found: {', '.join(issues)}", extra={"operation_id": event.operation_id})
            else:
                logger.info(f"Health check completed: {health_status}", extra={"operation_id": event.operation_id})
