...
class EventDrivenDatabase:
    ...
    def initialize(self) -> None:
        ...
        try:
            ...
            connection_event = DatabaseConnectionEvent(connection_status="connected", database_path=str(self.db_path))
            self.event_bus.publish(connection_event)

        except Exception as e:
            # Publish connection failure event
            connection_event = DatabaseConnectionEvent(connection_status="failed", database_path=str(self.db_path), error=str(e))
            self.event_bus.publish(connection_event)
            raise
    ...
    def clear_database(self) -> None:
        ...
        try:
            ...
            clear_event = DatabaseClearedEvent(
                records_deleted=records_to_delete, tables_cleared=["chunks", "embeddings", "documents", "file_metadata", "bm25_index", "bm25_statistics"]
            )
    ...
    def close(self) -> None:
        ...
        try:
            ...
            connection_event = DatabaseConnectionEvent(connection_status="disconnected", database_path=str(self.db_path))
            self.event_bus.publish(connection_event)
    ...
    def store_file_metadata(
        ...
        return self.database_service.store_file_metadata(path, file_size, file_modified_at, content_hash, chunk_count)
