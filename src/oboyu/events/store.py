...
class EventStore:
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                conn.execute(
                    ...
                    VALUES (?, ?, ?, ?)
                """,
                    (event.event_type, event.operation_id, event.timestamp.isoformat(), event_data),
                )
                conn.commit()
    ...
    def get_events_by_time_range(self, start_time: datetime, end_time: datetime, event_type: Optional[str] = None) -> List[dict]:
