...
class SnippetProcessor:
    ...
    def __init__(self, config: SnippetConfig) -> None:
        ...
        self.strategies: Dict[SnippetStrategy, SnippetStrategyBase] = {
            SnippetStrategy.FIXED_LENGTH: FixedLengthStrategy(self.context_provider, self.japanese_processor),
            SnippetStrategy.SENTENCE_BOUNDARY: SentenceBoundaryStrategy(self.context_provider, self.japanese_processor),
            SnippetStrategy.PARAGRAPH_BOUNDARY: ParagraphBoundaryStrategy(self.context_provider, self.japanese_processor),
        }
    ...
    def _find_query_matches(self, content: str, query: str) -> List[SnippetMatch]:
        ...
        for word in query_words:
            ...
            while True:
                ...
                if (pos == 0 or not content[pos - 1].isalnum()) and (pos + len(word) >= len(content) or not content[pos + len(word)].isalnum()):
                    match = SnippetMatch(
                        start=pos,
                        end=pos + len(word),
