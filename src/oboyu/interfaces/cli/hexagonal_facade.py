...
class HexagonalFacade:
    ...
    async def index_directory(self, directory_path: Path, include_patterns: Optional[List[str]] = None, exclude_patterns: Optional[List[str]] = None) -> None:
        """Index a directory."""
        await self.indexing_service.index_directory(directory_path, include_patterns or [], exclude_patterns or [])

    async def search(
        self, query_text: str, mode: str = "hybrid", top_k: int = 10, threshold: float = 0.0, language: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        ...
        query = Query(text=query_text, mode=search_mode, top_k=top_k, language=lang_code, similarity_threshold=threshold)
