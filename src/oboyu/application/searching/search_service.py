...
class SearchService:
    ...
    async def _vector_search(self, query: Query) -> List[SearchResult]:
        ...
        results = await self._search_repository.find_by_vector_similarity(query_vector=query_embedding, top_k=query.top_k, threshold=query.similarity_threshold)
    ...
    async def _hybrid_search(self, query: Query) -> List[SearchResult]:
        ...
        combined_results = self._search_engine.combine_results(vector_results, bm25_results, vector_weight=0.7, bm25_weight=0.3)
