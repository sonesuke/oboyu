...
class SearchResult:
    ...
    def has_context(self) -> bool:
        ...
        return bool((self.context_before is not None and self.context_before.strip()) or (self.context_after is not None and self.context_after.strip()))

    def is_high_quality(self) -> bool:
        """Determine if this is a high-quality search result."""
        return self.score.value >= 0.5 and len(self.content.strip()) >= 20 and self.language != LanguageCode.UNKNOWN

    def create_with_highlights(self, highlighted_content: str, search_terms: List[str]) -> "SearchResult":
    ...
    def create_with_context(self, context_before: Optional[str], context_after: Optional[str]) -> "SearchResult":
