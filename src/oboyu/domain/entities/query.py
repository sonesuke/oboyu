...
class Query:
    ...
    def should_use_vector_search(self) -> bool:
        ...
        return self.mode in (SearchMode.VECTOR, SearchMode.HYBRID) and len(self.get_normalized_text()) > 10

    def should_use_bm25_search(self) -> bool:
        """Determine if BM25 search would be beneficial."""
        return self.mode in (SearchMode.BM25, SearchMode.HYBRID) and len(self.get_terms()) >= 1

    def create_variant_with_mode(self, mode: SearchMode) -> "Query":
        """Create a variant of this query with a different search mode."""
        return Query(text=self.text, mode=mode, top_k=self.top_k, language=self.language, similarity_threshold=self.similarity_threshold)
