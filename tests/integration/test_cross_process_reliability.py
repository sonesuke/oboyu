...
def create_test_indexer_config(db_path: Path) -> IndexerConfig:
    ...
    return IndexerConfig(
        model=ModelConfig(embedding_model="cl-nagoya/ruri-v3-30m", batch_size=32),
        processing=ProcessingConfig(chunk_size=500, chunk_overlap=50, max_workers=2, db_path=db_path),
    )
...
def index_documents_in_process(db_path: str, test_dir: str) -> Dict[str, int]:
    ...
    try:
        ...
        for test_file in Path(test_dir).glob("*.txt"):
            ...
            crawler_results.append(CrawlerResult(path=test_file, title=f"Test Document - {test_file.name}", content=content, language="en", metadata={}))
        ...
        return {"success": False, "error": str(e), "process_id": os.getpid()}
...
def query_database_in_process(db_path: str, query: str) -> Dict[str, int]:
    ...
    try:
        ...
        return {"success": True, "result_count": len(results), "process_id": os.getpid()}
    except Exception as e:
...
def clear_database_in_process(db_path: str) -> Dict[str, int]:
    ...
    try:
        ...
        return {"success": True, "process_id": os.getpid()}
    except Exception as e:
