...
class SearchRepositoryContract:
    ...
    def sample_query(self) -> Query:
        ...
        return Query(text="Python programming", mode=SearchMode.BM25, top_k=10, language=LanguageCode.ENGLISH, similarity_threshold=0.0)

    # Basic Storage and Retrieval Contracts

    async def test_store_and_retrieve_single_chunk(self, repository: SearchRepository, sample_chunk: Chunk):
    ...
    async def test_store_and_retrieve_multiple_chunks(self, repository: SearchRepository, sample_chunks: List[Chunk]):
    ...
    async def test_chunk_exists_behavior(self, repository: SearchRepository, sample_chunk: Chunk):
    ...
    async def test_store_and_search_by_embedding(self, repository: SearchRepository, sample_chunk: Chunk, sample_embedding: EmbeddingVector):
        ...
        results = await repository.find_by_vector_similarity(query_vector=sample_embedding, top_k=5, threshold=0.0)

        # Should find the stored chunk
        assert len(results) > 0
        assert any(result.chunk_id == sample_chunk.id for result in results)

    async def test_store_multiple_embeddings(self, repository: SearchRepository, sample_chunks: List[Chunk]):
        ...
        for chunk_id, embedding in embeddings:
            results = await repository.find_by_vector_similarity(query_vector=embedding, top_k=1, threshold=0.0)
            assert len(results) > 0
            assert results[0].chunk_id == chunk_id

    # BM25 Search Contracts

    async def test_bm25_search_basic(self, repository: SearchRepository, sample_chunks: List[Chunk], sample_query: Query):
    ...
    async def test_delete_single_chunk(self, repository: SearchRepository, sample_chunk: Chunk):
        """Contract: Delete single chunk removes all associated data."""
        # Store chunk
        await repository.store_chunk(sample_chunk)
        await repository.store_embedding(sample_chunk.id, EmbeddingVector.create([0.1, 0.2, 0.3]))
    ...
    async def test_delete_chunks_by_document(self, repository: SearchRepository, sample_chunks: List[Chunk]):
    ...
    async def test_chunk_count_accuracy(self, repository: SearchRepository, sample_chunks: List[Chunk]):
    ...
    async def test_embedding_count_accuracy(self, repository: SearchRepository, sample_chunks: List[Chunk]):
        ...
        # Store embeddings
        embeddings = [(chunk.id, EmbeddingVector.create([0.1, 0.2, 0.3])) for chunk in sample_chunks]
        await repository.store_embeddings(embeddings)
    ...
    async def test_duplicate_chunk_storage(self, repository: SearchRepository, sample_chunk: Chunk):
    ...
    async def test_concurrent_access_safety(self, repository: SearchRepository, sample_chunks: List[Chunk]):
    ...
    async def test_search_result_ordering(self, repository: SearchRepository, sample_chunks: List[Chunk]):
        """Contract: Search results are ordered by relevance (descending score)."""
        # Store chunks
        await repository.store_chunks(sample_chunks)

        query = Query(text="programming", mode=SearchMode.BM25, top_k=10)
    ...
    async def test_top_k_limit_respected(self, repository: SearchRepository, sample_chunks: List[Chunk]):
        ...
        top_k = 2
        query = Query(text="test", mode=SearchMode.BM25, top_k=top_k)
    ...
    async def test_similarity_threshold_respected(self, repository: SearchRepository, sample_chunk: Chunk, sample_embedding: EmbeddingVector):
...
async def run_all_contracts(repository: SearchRepository) -> None:
    ...
    test_methods = [method for method in dir(contract) if method.startswith("test_") and callable(getattr(contract, method))]
