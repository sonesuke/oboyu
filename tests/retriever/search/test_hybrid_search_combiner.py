...
def test_rrf_scoring_order():
    ...
    vector_results = [
        SearchResult(chunk_id=f"v{i}", path=f"/v{i}.txt", title=f"V{i}", content=f"Vector {i}", chunk_index=0, language="en", score=1.0 - i * 0.1)
        for i in range(5)
    ]

    bm25_results = [
        SearchResult(chunk_id=f"b{i}", path=f"/b{i}.txt", title=f"B{i}", content=f"BM25 {i}", chunk_index=0, language="en", score=1.0 - i * 0.1)
        for i in range(5)
    ]
