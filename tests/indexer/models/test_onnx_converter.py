...
class TestONNXEmbeddingModel:
    ...
    def test_init_with_valid_model(self, tmp_path: Path) -> None:
        ...
        with (
            patch("oboyu.common.onnx.embedding_model.InferenceSession") as mock_session,
...
class TestONNXConversion:
    ...
    def test_get_or_convert_onnx_model_new(self, mock_cache: MagicMock, tmp_path: Path) -> None:
        ...
        mock_cache.assert_called_once_with(model_name, "embedding", cache_dir=cache_dir, apply_quantization=True, quantization_config=None)
...
class TestONNXQuantization:
    ...
    @patch("oboyu.common.onnx.embedding_model.SentenceTransformer")
    @patch("oboyu.common.onnx.embedding_model.torch.onnx.export")
    def test_convert_to_onnx_with_quantization(self, mock_export: MagicMock, mock_st: MagicMock, mock_quantize: MagicMock, tmp_path: Path) -> None:
    ...
    def test_get_or_convert_onnx_model_with_quantization(self, mock_cache: MagicMock, tmp_path: Path) -> None:
        ...
        result = get_or_convert_onnx_model(model_name, cache_dir, apply_quantization=True, quantization_config=quant_config)

        # Verify cache was called with quantization config
        mock_cache.assert_called_once_with(model_name, "embedding", cache_dir=cache_dir, apply_quantization=True, quantization_config=quant_config)
