...
def test_rank_based_normalization(sample_results):
    ...
    assert min(scores) == 1/3   # Worst rank gets 1/n
...
def test_unknown_normalization_method():
    ...
    with patch("oboyu.indexer.core.score_normalizer.logger") as mock_logger:
...
@patch("oboyu.indexer.core.score_normalizer.logger")
