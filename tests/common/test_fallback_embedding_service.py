...
class TestFallbackEmbeddingService:
    ...
    def test_initialization_with_primary_service_param(self, mock_embedding_service):
        ...
        service = FallbackEmbeddingService(primary_service=mock_primary, enable_fallback_services=False, enable_local_fallback=False)
    ...
    def test_initialization_fallback_services(self, mock_embedding_service):
        ...
        service = FallbackEmbeddingService(model_name="test-model", fallback_model_names=["fallback1", "fallback2"])
    ...
    def test_generate_query_embedding(self):
        ...
        service = FallbackEmbeddingService(primary_service=mock_primary, use_circuit_breaker=False, query_prefix="検索: ")
    ...
    def test_get_circuit_breaker_status(self, mock_registry):
        ...
        mock_primary_cb.get_metrics.return_value = Mock(
            total_requests=10, successful_requests=8, failed_requests=2, rejected_requests=0, get_failure_rate=Mock(return_value=0.2)
        )

        mock_fallback_cb = Mock()
        mock_fallback_cb.get_state.return_value = CircuitState.OPEN
        mock_fallback_cb.get_metrics.return_value = Mock(
            total_requests=5, successful_requests=0, failed_requests=5, rejected_requests=3, get_failure_rate=Mock(return_value=1.0)
        )
        ...
        service = FallbackEmbeddingService(primary_service=mock_primary, model_name="primary-model")
        service.fallback_services = [mock_fallback]
    ...
    def test_disable_fallback(self):
        ...
        mock_primary = Mock()
        service = FallbackEmbeddingService(primary_service=mock_primary, enable_fallback_services=False, enable_local_fallback=False, use_circuit_breaker=False)
