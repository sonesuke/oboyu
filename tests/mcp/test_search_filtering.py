...
class TestMCPSearchFiltering:
    ...
    def test_search_without_filters(self, temp_db: str) -> None:
        ...
        result = search(query="test query", mode="hybrid", top_k=5, db_path=temp_db)
    ...
    def test_search_with_empty_filters(self, temp_db: str) -> None:
        ...
        result = search(query="test query", mode="hybrid", top_k=5, db_path=temp_db, filters={})
    ...
    def test_search_with_date_range_filter(self, temp_db: str) -> None:
        ...
        filters = {"date_range": {"start": "2024-01-01", "end": "2024-12-31", "field": "modified_at"}}

        result = search(query="test query", mode="hybrid", top_k=5, db_path=temp_db, filters=filters)
    ...
    def test_search_with_path_filter(self, temp_db: str) -> None:
        ...
        filters = {"path_filter": {"include_patterns": ["*/docs/*", "*.md"], "exclude_patterns": ["*/test/*", "*.log"]}}
    ...
    def test_search_with_combined_filters(self, temp_db: str) -> None:
        ...
        filters = {
            "date_range": {"start": "2024-06-01", "field": "modified_at"},
            "path_filter": {"include_patterns": ["*/docs/*"], "exclude_patterns": ["*/archived/*"]},
        }
    ...
    def test_search_with_invalid_filters(self, temp_db: str) -> None:
        ...
        filters = {"date_range": {"start": "invalid-date", "field": "invalid_field"}}
    ...
    def test_search_filters_integration_with_snippet_config(self, temp_db: str) -> None:
        ...
        snippet_config = {"max_length": 200, "show_surrounding_context": True}

        result = search(query="test query", mode="hybrid", top_k=5, db_path=temp_db, filters=filters, snippet_config=snippet_config)
    ...
    def test_search_filters_with_all_modes(self, temp_db: str) -> None:
        ...
        filters = {"path_filter": {"include_patterns": ["*.md"]}}

        # Test vector mode
        result_vector = search(query="test query", mode="vector", top_k=5, db_path=temp_db, filters=filters)
        assert "results" in result_vector

        # Test BM25 mode
        result_bm25 = search(query="test query", mode="bm25", top_k=5, db_path=temp_db, filters=filters)
        assert "results" in result_bm25

        # Test hybrid mode
        result_hybrid = search(query="test query", mode="hybrid", top_k=5, db_path=temp_db, filters=filters)
    ...
    def test_search_filters_with_language_filter(self, temp_db: str) -> None:
        ...
        result = search(query="テストクエリ", mode="hybrid", top_k=5, language="ja", db_path=temp_db, filters=filters)
...
class TestSearchFiltersValidation:
    ...
    def test_date_range_filter_validation(self) -> None:
        ...
        # Valid filter
        valid_filter = {"date_range": {"start": "2024-01-01", "end": "2024-12-31", "field": "modified_at"}}
    ...
    def test_path_filter_validation(self) -> None:
        ...
        # Valid filter
        valid_filter = {"path_filter": {"include_patterns": ["*/docs/*"], "exclude_patterns": ["*/test/*"]}}
        filters = SearchFilters.from_dict(valid_filter)
...
class TestSearchFiltersExamples:
    ...
    def test_japanese_date_range_example(self, temp_db: str) -> None:
        ...
        result = search(query="機械学習アルゴリズム", mode="hybrid", top_k=5, db_path=temp_db, filters=filters)
    ...
    def test_api_documentation_path_example(self, temp_db: str) -> None:
        ...
        filters = {"path_filter": {"include_patterns": ["*/docs/*", "*/api/*", "*.md"], "exclude_patterns": ["*/test/*", "*/temp/*", "*.log"]}}

        result = search(query="API documentation", mode="hybrid", top_k=5, db_path=temp_db, filters=filters)
    ...
    def test_combined_filtering_example(self, temp_db: str) -> None:
        ...
        filters = {
            "date_range": {"start": "2024-06-01", "field": "modified_at"},
            "path_filter": {"include_patterns": ["*/documentation/*"], "exclude_patterns": ["*/archived/*"]},
        }

        result = search(query="設計パターン", mode="hybrid", top_k=5, db_path=temp_db, filters=filters)
    ...
    def test_recent_documents_example(self, temp_db: str) -> None:
        ...
        filters = {"date_range": {"start": "2024-05-01", "field": "modified_at"}, "path_filter": {"include_patterns": ["*/docs/*"]}}

        result = search(query="システム設計", mode="hybrid", top_k=5, db_path=temp_db, filters=filters)
    ...
    def test_project_specific_search_example(self, temp_db: str) -> None:
        ...
        filters = {"path_filter": {"include_patterns": ["*/backend/*", "*/api/*"], "exclude_patterns": ["*/test/*", "*/deprecated/*"]}}

        result = search(query="API implementation", mode="hybrid", top_k=5, db_path=temp_db, filters=filters)
