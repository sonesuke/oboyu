...
class TestSnippetIntegration:
    ...
    def test_search_with_snippet_config(self, monkeypatch):
        ...
        snippet_config = {"length": 100, "highlight_matches": True, "strategy": "sentence_boundary"}

        result = search(query="machine learning", snippet_config=snippet_config)
    ...
    def test_search_without_snippet_config(self, monkeypatch):
        ...
        mock_search_result = SearchResult(
            chunk_id="test_chunk", path="/test/path.txt", title="Test Document", content=original_content, chunk_index=0, language="en", metadata={}, score=0.85
        )
        mock_retriever.search.return_value = [mock_search_result]
    ...
    def test_search_with_invalid_snippet_config(self, monkeypatch, caplog):
        ...
        result = search(query="test query", snippet_config=invalid_snippet_config)
    ...
    def test_search_with_japanese_snippet_config(self, monkeypatch):
        ...
        snippet_config = {"length": 150, "japanese_aware": True, "strategy": "sentence_boundary", "prefer_complete_sentences": True, "highlight_matches": False}

        result = search(query="機械学習", snippet_config=snippet_config)
    ...
    def test_search_with_multi_level_snippet_config(self, monkeypatch):
        ...
        mock_search_result = SearchResult(
            chunk_id="test_chunk", path="/test/path.txt", title="Long Document", content=long_content, chunk_index=0, language="en", metadata={}, score=0.85
        )
        mock_retriever.search.return_value = [mock_search_result]
        ...
        snippet_config = {
            ...
            "levels": [{"type": "summary", "length": 50}, {"type": "detailed", "length": 150}],
