...
SAMPLE_DATA = {
    ...
    "weather": {"japanese": ["晴れ", "曇り", "雨", "雪", "快晴"], "english": ["Sunny", "Cloudy", "Rainy", "Snowy", "Clear"]},
    "subjects": {
        "japanese": ["新システム導入について", "会議のお知らせ", "プロジェクト進捗報告", "研修プログラムのご案内", "セキュリティアップデート"],
        "english": ["New System Implementation", "Meeting Announcement", "Project Progress Report", "Training Program Notice", "Security Update"],
    },
    "projects": {"japanese": ["次世代AI", "スマートシティ", "グリーンエネルギー"], "english": ["Next-Gen AI", "Smart City", "Green Energy"]},
}
...
def generate_dataset(size: str, output_dir: Path, clean: bool = True) -> Dict[str, int]:
    ...
    stats = {"total_files": 0, "total_size": 0, "files_by_type": {}}
...
def main() -> None:
    ...
    parser.add_argument("sizes", nargs="+", choices=list(DATASET_SIZES.keys()) + ["all"], help="Dataset sizes to generate")
    parser.add_argument("--output-dir", type=Path, default=DATA_DIR, help="Output directory for datasets")
    parser.add_argument("--no-clean", action="store_true", help="Don't clean existing data before generating")
