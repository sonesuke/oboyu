...
class BenchmarkReporter:
    ...
    def generate_text_report(self) -> str:
        ...
        if self.run.indexing_results:
            ...
            for size, result in sorted(self.run.indexing_results.items()):
                ...
                lines.append(
                    f"    Content Extraction: {format_time(result.content_extraction_time)} ({result.content_extraction_time / result.total_time * 100:.1f}%)"
                )
                lines.append(
                    f"    Embedding Generation: {format_time(result.embedding_generation_time)} ({result.embedding_generation_time / result.total_time * 100:.1f}%)"
                )
                lines.append(
                    f"    Database Storage: {format_time(result.database_storage_time)} ({result.database_storage_time / result.total_time * 100:.1f}%)"
                )
                lines.append("")
    ...
    def print_summary(self) -> None:
        ...
        if self.run.search_results:
            ...
            for key, result in sorted(self.run.search_results.items()):
                dataset, language = key.rsplit("_", 1)
                row = [dataset.upper(), language.capitalize(), str(result.total_queries)]
