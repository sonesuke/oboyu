...
class SearchBenchmark:
    ...
    def __init__(self, db_path: Path, query_language: str = "japanese", queries_dir: Path = QUERIES_DIR) -> None:
        ...
        config = IndexerConfig(processing=ProcessingConfig(db_path=db_path))
    ...
    def _run_single_search(self, query_text: str, top_k: int, warm_cache: bool = False) -> Tuple[float, int, Optional[float]]:
    ...
    def run(self, test_queries: Optional[int] = None, monitor_system: bool = True) -> SearchBenchmarkResult:
        ...
        for top_k in self.config["top_k_values"]:
            ...
            for query in track(queries_to_test, description=f"Running queries (k={top_k})"):
                # Run search
                response_time, result_count, first_result_time = self._run_single_search(query["text"], top_k=top_k)
        ...
        result = SearchBenchmarkResult(
            ...
            metadata={"dataset_info": self.dataset_info, "warmup_runs": self.config["warmup_runs"], "test_runs": len(queries_to_test)},
        )
...
def benchmark_search(
    db_paths: Dict[str, Path], query_languages: List[str], queries_dir: Path = QUERIES_DIR, test_queries: Optional[int] = None, monitor_system: bool = True
...
if __name__ == "__main__":
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        results = benchmark_search({"test": db_path}, ["english"], test_queries=5)
