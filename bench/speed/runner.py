...
class BenchmarkRunner:
    ...
    def __init__(
        self, dataset_sizes: List[str], query_languages: List[str], data_dir: Path = DATA_DIR, queries_dir: Path = QUERIES_DIR, results_dir: Path = RESULTS_DIR
    ) -> None:
    ...
    def prepare_data(self, force_regenerate: bool = False) -> bool:
        ...
        queries_exist = all((self.queries_dir / f"{lang}_queries.json").exists() for lang in self.query_languages)
        ...
        if not queries_exist or force_regenerate:
            ...
            filtered_queries = {lang: q for lang, q in queries.items() if lang in self.query_languages}
    ...
    def run_indexing_benchmarks(self) -> Dict[str, any]:
        ...
        results = benchmark_indexing(self.dataset_sizes, self.data_dir, monitor_system=True)
    ...
    def create_test_indexes(self) -> Dict[str, Path]:
        ...
        for size in self.dataset_sizes:
            ...
            indexer_dict = {**OBOYU_CONFIG["indexer"], "db_path": str(db_path)}
    ...
    def run(self, force_regenerate: bool = False, skip_indexing: bool = False, skip_search: bool = False, use_existing_indexes: bool = False) -> BenchmarkRun:
        ...
        run = BenchmarkRun(
            ...
            config={"dataset_sizes": self.dataset_sizes, "query_languages": self.query_languages, "oboyu_config": OBOYU_CONFIG},
...
def run_quick_benchmark() -> BenchmarkRun:
    ...
    return runner.run(force_regenerate=False, skip_indexing=False, skip_search=False, use_existing_indexes=False)
