...
class IndexingBenchmark:
    ...
    def _run_single_indexing(self, show_progress: bool = True) -> Dict[str, float]:
        ...
        try:
            ...
            crawler_config = CrawlerConfig(
                depth=10, max_workers=4, max_file_size=10 * 1024 * 1024, follow_symlinks=False, include_patterns=["*"], exclude_patterns=[]
            )
            ...
            if show_progress:
                progress = Progress(
                    SpinnerColumn(), TextColumn("[progress.description]{task.description}"), TimeElapsedColumn(), console=console, transient=True
                )
            ...
            indexer_config = IndexerConfig(processing=ProcessingConfig(db_path=db_path), model=ModelConfig(embedding_model="cl-nagoya/ruri-v3-30m"))
            ...
            metrics["total_time"] = metrics["file_discovery_time"] + metrics["content_extraction_time"] + total_indexing_time
    ...
    def _aggregate_results(self, all_metrics: List[Dict[str, float]]) -> IndexingResult:
        ...
        return IndexingResult(
            ...
            metadata={"warmup_runs": self.warmup_runs, "test_runs": self.test_runs, "all_runs": all_metrics},
        )


def benchmark_indexing(dataset_sizes: List[str], data_dir: Path = DATA_DIR, monitor_system: bool = True) -> Dict[str, IndexingResult]:
