...
class BenchmarkAnalyzer:
    ...
    def find_regressions(self, baseline_run: BenchmarkRun, current_run: BenchmarkRun, threshold: float = 0.1) -> Dict[str, List[Dict[str, Any]]]:
        """Find performance regressions between runs."""
        regressions = {"indexing": [], "search": []}
                ...
                if baseline_fps > 0:
                    ...
                    if change < -threshold:
                        regressions["indexing"].append(
                            {"dataset": size, "metric": "files_per_second", "baseline": baseline_fps, "current": current_fps, "change_percent": change * 100}
                        )
            ...
            if trends["indexing"]:
                ...
                for metric, values in sorted(trends["indexing"].items()):
                    if len(values) >= 2:
                        ...
                        table.add_row(metric, f"{first:.2f}", f"{latest:.2f}", f"{change:+.1f}%", trend)
...
def main() -> None:
    ...
    parser.add_argument("--results-dir", type=Path, default=RESULTS_DIR, help="Directory containing benchmark results")
    parser.add_argument("--latest", type=int, default=5, help="Number of latest runs to analyze")
    parser.add_argument("--compare", nargs=2, help="Compare two specific runs by ID")
    parser.add_argument("--regression-threshold", type=float, default=0.1, help="Threshold for regression detection (default: 10%%)")
