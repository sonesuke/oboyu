...
class BenchmarkRun:
    ...
    def save(self, output_dir: Path = RESULTS_DIR) -> Path:
        ...
        summary = {
            ...
            "system": {"platform": self.system_info.platform, "python": self.system_info.python["version"].split()[0], "oboyu": self.system_info.oboyu_version},
            "indexing": {
                size: {"total_time": result.total_time, "files_per_second": result.files_per_second, "total_files": result.total_files}
    ...
    def load(cls, filepath: Path) -> "BenchmarkRun":
        ...
        indexing_results = {size: IndexingResult(**result) for size, result in data.get("indexing_results", {}).items()}

        search_results = {}
        for key, result in data.get("search_results", {}).items():
            # Convert individual results
            individual_results = [SearchResult(**r) for r in result.get("individual_results", [])]
...
class ResultsManager:
    ...
    def compare_runs(self, run_ids: List[str]) -> Dict[str, Any]:
        ...
        comparison = {
            "runs": [{"run_id": run.run_id, "timestamp": run.timestamp, "system": run.system_info.platform} for run in runs],
            "indexing": {},
            "search": {},
        }
        ...
        for key in runs[0].search_results.keys():
            if all(key in run.search_results for run in runs):
                ...
                comparison["search"][key] = {"avg_response_time": all_response_times}
